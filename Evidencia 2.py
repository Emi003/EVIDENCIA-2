...
while True:
    ...
    if opcion == "1":
        ...
        print("\nNota creada exitosamente. Especificaciones:")
        print(f"Folio: {folio_actual}")
        print(f"Nombre del Cliente: {Nombre_Cliente}")
        print(f"RFC del Cliente: {RFC_Cliente}")
        print(f"Correo Electrónico del Cliente: {Correo_Cliente}")
        print(f"Fecha: {fecha_procesada.strftime('%d/%m/%Y')}")
        print("Servicios:")
        for i, (servicio, monto) in enumerate(servicios_nota, start=1):
            print(f"   {i}. {servicio}: ${monto:.2f}")
        ...
        while True:
            ...
            opcion_consultas = input("Seleccione una opción (1/2/3/4): ").strip()
                ...
                if rfc_seleccionado.lower() == "no":
                    ...
                    if 0 < rfc_seleccionado <= len(rfc_list):
                        ...
                        for nota in rfc_clientes[rfc_elegido]:
                             for i, (servicio, monto) in enumerate(nota['Servicios'], start=1):
                                  data.append([nota['Folio'], nota['Nombre_Cliente'], nota['Fecha'], servicio, f"${monto:.2f}"])
                        df = pd.DataFrame(data, columns=["Folio", "Nombre Cliente", "Fecha", "Servicio", "Monto"])
        ...
        if folio_cancelar.isdigit() and int(folio_cancelar) in [nota["Folio"] for nota in notas]:
            ...
            if folio_cancelar in notas_canceladas:
                ...
                nota = next((nota for nota in notas if nota["Folio"] == folio_cancelar), None)
                if nota:
                    ...
                    confirmacion = input("¿Desea cancelar esta nota completa? (Sí/No): ").strip().lower()
                    if confirmacion == "si":
                    ...
                    print("El folio no corresponde a una nota existente.")
        else:
        ...
        if notas_canceladas:
            for folio in notas_canceladas:
                ...
                if nota:
                    print(f"Folio: {folio}, Nombre del Cliente: {nota['Nombre_Cliente']}")
            ...
            if folio_recuperar.lower() == "no":
                ...
                if folio_recuperar in notas_canceladas:
                    nota = next((nota for nota in notas if nota["Folio"] == folio_recuperar), None)
                    if nota:
